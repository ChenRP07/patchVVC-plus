	...
	void PVVCSegmentation::Segmentation() {
		try {
			...
			while (current_frame_idx_ < this->frames_.size()) {
				...
				fmt_1 % common::AZURE % common::BLUE % patch_size.size() % avg % (*std::min_element(patch_size.begin(), patch_size.end())) % (*std::max_element(patch_size.begin(), patch_size.end())) %
				...
				for (; current_frame_idx_ < this->frames_.size(); current_frame_idx_++) {
					...
					boost::format icp_fmt_1{"\033[%1%mICP segmentation finished\n"
					                        ...
					                        "\033[%1%m-------------------------------------------------------------------\n\033[0m"};
					icp_fmt_1 % common::AZURE % common::BLUE % cnt_stat % icp_stat.size() % avg_stat % max_stat % min_stat % this->clock_.GetTimeS() % this->clock_.GetTimeMs();
					std::cout << icp_fmt_1;
				}
			}
		}
