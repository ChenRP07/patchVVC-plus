...
namespace io {
	...
	void SaveColorPlyFile(const std::string& file_name, const pcl::PointCloud<pcl::PointXYZRGB>::Ptr point_cloud, bool binary_mode) {
		try {
			// file_name must be *.ply format
			std::regex  name_type{"^.*\\.ply$"};
			std::smatch matches;
	...
	void SaveUniqueColorPlyFile(const std::string& file_name, const pcl::PointCloud<pcl::PointXYZRGB>::Ptr point_cloud, unsigned int unique_color, bool binary_mode) {
		try {
			// file_name must be *.ply format
			std::regex  name_type{"^.*\\.ply$"};
			std::smatch matches;
