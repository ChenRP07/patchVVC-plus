...
namespace registration {
	...
	void PatchesRegistration::Task() {
		while (true) {
			...
			icp->SetSourceCloud(this->source_patches_[task_idx].cloud);
			icp->SetTargetCloud(this->target_patches_[task_idx].cloud);
			icp->Align();
			if (icp->Converged()) {
				this->mses_[task_idx] = icp->CloudMSE();
			}
			else {
				this->mses_[task_idx] = -1.0f;
			}
		}
	}
